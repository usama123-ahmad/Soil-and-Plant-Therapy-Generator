...
interface FoliarSprayProps {
  ...
  cardColor?: 'green' | 'orange';
}

const FoliarSpray: React.FC<FoliarSprayProps> = ({ selectedProducts, setSelectedProducts, deficientNutrients, cardColor = 'green' }) => {
            ...
            {selectedProducts.length > 0 && (
              <div className="space-y-2">
                ...
                {selectedProducts.map((product) => {
                  ...
                  const cardBg = cardColor === 'orange' ? 'bg-orange-100 border-l-4 border-orange-500' : 'bg-green-100 border-l-4 border-green-600';
                  const rateText = cardColor === 'orange' ? 'text-orange-700' : 'text-green-700';
                  return (
                    <Card key={product.id} className={cardBg}>
                      <CardContent className="pt-4">
                        <div className="flex items-center justify-between">
                          <div className="flex items-start space-x-3">
                            <div className="flex-1">
                              ...
                              <p className={`text-sm ${rateText}`}>Rate: {product.rate} {product.unit}</p>
                            </div>
                          </div>
                          <Button
                            variant="ghost"
